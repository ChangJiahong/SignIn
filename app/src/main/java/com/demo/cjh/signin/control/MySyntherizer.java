package com.demo.cjh.signin.control;
...
import com.demo.cjh.signin.MainHandlerConstant;
...
public class MySyntherizer implements MainHandlerConstant {
    ...
    public MySyntherizer(Context context, InitConfig initConfig) throws Exception {
        this(context);
        if(!init(initConfig)) {
            Log.d("My","鉴权失败");
            release();
            throw new Exception("鉴权失败，请检查网络后再试！");
        }
    }
    ...
    protected boolean init(InitConfig config) {
        ...
        SharedPreferences sp = App.Companion.getApp().getSp();
        if (isMix && !sp.getBoolean("auth",false)) {
        ...
        if (result != 0) {
            print("【error】initTts 初始化失败 + errorCode：" + result);
            return false;
        }
